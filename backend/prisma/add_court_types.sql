...
-- AlterTable - Adicionar courtTypeId na tabela courts
ALTER TABLE "courts" ADD COLUMN "courtTypeId" TEXT;

-- Migrar dados existentes (criar tipos personalizados para sportType existentes)
INSERT INTO "court_types" ("id", "name", "isDefault", "complexId", "createdAt", "updatedAt")
SELECT 
  gen_random_uuid(),
  c."sportType",
...
WHERE NOT EXISTS (
  ...
  WHERE ct."name" = c."sportType" AND (ct."complexId" = c."complexId" OR ct."isDefault" = true)
)
